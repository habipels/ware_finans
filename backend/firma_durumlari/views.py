...
def firma_ekleme(request):
    ...
    if request.method == "POST":
        ...
        yeni_sube = sube.objects.create(
            bagli_oldugu_firma = get_object_or_none(firma,id = firma_olustur.id ),
            sube_adi = sube_adi,sube_unvani = sube_soyadi,adres_bilgisi = get_object_or_none(adresler,id = yeni_adres.id),
            vergi_dairesi_adi =get_object_or_none(vergi_dairesi,vergi_dairesi_adi = firmavergidairesi) ,vergi_dairesi_kodu = firmavergidairesikodu,
        ...
        sube_faliyet_bilgileri.objects.create(sube_bilgisi = get_object_or_none(sube,id = yeni_sube.id),
        kurulus_tarihi = faliyet_kurulus_tarihi ,terk_tarihi = faliyet_terk_tarihi 
        ,faliyet_nace_kodu = get_object_or_none(faliyet_bilgisi,faliyet_kodu = faliyetnacekodu ),
        tabi_oldugu_sektor = faliyetsektoru ,faliyet_niteligi = faliyetniteligi ,yapilan_is_niteligi = yapilanisinniteligi ,
        calisma_ve_sosyal_is_kolu = get_object_or_none(calisma_sosyal_guvenlik_is_kollari,id = faliyetcalismasgi ),
        ...
        isyeri_mulkiyet = faliyetisyerimulkiyet ,vergi_dairesi_adi = get_object_or_none(vergi_dairesi,vergi_dairesi_adi =faliyetvergidairesikodu  ),
        ...
        ihale_bilgileri.objects.create(sube_bilgisi = get_object_or_none(sube,id = yeni_sube.id),
        ...
        taseronbilgileri.objects.create(sube_bilgisi = get_object_or_none(sube,id = yeni_sube.id),
        taseronunvanbilgisi =  taseronunvanbilgisi ,adres_bilgisi =   get_object_or_none(adresler,id = yeni_adres_taseron.id),
        ...
        kurumlar_dar_mukkelef_kimlik_ve_adres_bilgisi.objects.create(
          sube_bilgisi = get_object_or_none(sube,id = yeni_sube.id),
        ...
        beyanname_verilme = beyanname_bilgileri.objects.create(sube_bilgisi = get_object_or_none(sube,id = yeni_sube.id),
        ...
        beyanname_kanuni_temsilcisi.objects.create(
            beyanname_bilgisi = get_object_or_none(beyanname_bilgileri,id = beyanname_verilme.id),
        ...
        sgk_bilgileri.objects.create(
            ...
            personel_girisinde_otomatik_kanun = get_object_or_none(sgk_kanunlari,id = personel_girisinde_otomatik_kanun),
            personel_6116645yararlanmiyorsayenikanun = get_object_or_none(sgk_kanunlari,id = personel_6116645yararlanmiyorsayenikanun)
        )
...
def firma_gosterme(request,slug):
    content = site_ayarlari()
    kayitli_firmam = get_object_or_none(firma,silinme_bilgisi = False,firma_ozel_anahtar = slug)
    ...
    content["adres"] = adres
    content["faliyet_bilgisi"] =  sube_faliyet_bilgileri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    content["ihale"] = ihale_bilgileri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    content["taseronbilgileri"] = taseronbilgileri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    content["kurumlardar"] = kurumlar_dar_mukkelef_kimlik_ve_adres_bilgisi.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    ...
    content["subeleri"] =  sube.objects.filter(silinme_bilgisi = False,bagli_oldugu_firma = get_object_or_none(firma,silinme_bilgisi = False,firma_muhasabecisi = request.user,firma_ozel_anahtar = slug))
    content["sgk_bigileri"] = sgk_bilgileri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    content["sgkbm"] = sgk_bolgecalisma_mudurlukleri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    content["iskur"] = iskurbilgileri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    sgkpersonelkanun = sgk_kanunlari.objects.all()
    content["sgkpersonelkanun"]=sgkpersonelkanun
    if request.method == "POST":
        tanitici_isim = request.POST.get("firmataniticiadi")
        firma_adi = request.POST.get("firmaunvanadi")
        firma_soyadi = request.POST.get("firmaunvansoyadi")
        ...
        if kdv2 == "kdv2aylik":
            ...
            kdv2 = "Üç Aylık"
        firma_olustur = firma.objects.filter(firma_ozel_anahtar = slug).update(firma_muhasabecisi = request.user,
                             tanitici_isim = tanitici_isim ,
                             firma_unvani = firma_adi,
                             Firma_unvani2 = firma_soyadi,
                             silinme_bilgisi = False)
        ...
        yeni_sube = sube.objects.filter(bagli_oldugu_firma = get_object_or_none(firma,firma_ozel_anahtar = slug )).update(
        ...
        yeni_sube = sube.objects.filter(bagli_oldugu_firma = get_object_or_none(firma,firma_ozel_anahtar = slug )).last()
        sube_faliyet_bilgileri.objects.create(sube_bilgisi = get_object_or_none(sube,id = yeni_sube.id),
        kurulus_tarihi = faliyet_kurulus_tarihi ,terk_tarihi = faliyet_terk_tarihi 
        ,faliyet_nace_kodu = get_object_or_none(faliyet_bilgisi,faliyet_kodu = faliyetnacekodu ),
        tabi_oldugu_sektor = faliyetsektoru ,faliyet_niteligi = faliyetniteligi ,yapilan_is_niteligi = yapilanisinniteligi ,
...
def firmaya_sube_ekleme(request,slug):
    ...
    content = {"form":form,"vergidaireleri":v,
               ...
               "cvsgik":cvsgik,"firmalrim":kayitli_firmalarim,"sgkpersonelkanun":sgkpersonelkanun}
    kayitli_firmam = get_object_or_none(firma,silinme_bilgisi = False,firma_ozel_anahtar = slug)
    sube_bilgisi = sube.objects.filter(bagli_oldugu_firma =  kayitli_firmam,silinme_bilgisi = False)
    content["kayitlifirmam"]= kayitli_firmam
    content["sube_bilgisi"] = sube_bilgisi
    if request.method == "POST":
        tanitici_isim = request.POST.get("firmataniticiadi")
        ...
        yeni_sube = sube.objects.create(
            bagli_oldugu_firma = get_object_or_none(firma,firma_ozel_anahtar = slug ),
            sube_adi = sube_adi,sube_unvani = sube_soyadi,adres_bilgisi = get_object_or_none(adresler,id = yeni_adres.id),
            vergi_dairesi_adi =get_object_or_none(vergi_dairesi,vergi_dairesi_adi = firmavergidairesi) ,vergi_dairesi_kodu = firmavergidairesikodu,
            vergi_numarasi = firmaverginumarasi,sahis_ise_tc = sahisisetc,
            email_adresi = firmaeposta,web_adresi = firmawebadresi,
...
def sube_sil(request,slug,id):
    sube.objects.filter(bagli_oldugu_firma__firma_ozel_anahtar = slug,id = id).update(silinme_bilgisi = True)
    a = "/company/show/"+slug
    return redirect(a)

def firma_sil(request,slug):
    firma.objects.filter(firma_ozel_anahtar = slug).update(silinme_bilgisi = True)
    a = "/company/companysettings/"
    return redirect(a)

def firma_ortak_eklme_silme(request,slug):
    content = site_ayarlari()
    kayitli_firmam = get_object_or_none(firma,silinme_bilgisi = False,firma_ozel_anahtar = slug)
    sube_bilgisi = sube.objects.filter(bagli_oldugu_firma =  kayitli_firmam).first()
    adres = adresler.objects.filter(id  = sube_bilgisi.adres_bilgisi.id ).first()
    ...
    content["sgkbm"] = sgk_bolgecalisma_mudurlukleri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    content["iskur"] = iskurbilgileri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    content["ortaklar"] = ortak_bilgileri.objects.filter(firma_bilgisi = get_object_or_none( firma,silinme_bilgisi = False,firma_ozel_anahtar = slug))
    sgkpersonelkanun = sgk_kanunlari.objects.all()
    content["sgkpersonelkanun"]=sgkpersonelkanun
    if request.POST:
        unvanbilgisi = request.POST.get("unvanbilgisi")
        ortakvekil = request.POST.get("ortakvekil")
        adisoyadi = request.POST.get("adisoyadi")
        otaklikgiristarihi = request.POST.get("otaklikgiristarihi")
        otaklikcikistarihi =request.POST.get("otaklikcikistarihi")
        gorevbitistarihi = request.POST.get("gorevbitistarihi")
        hisseorani = request.POST.get("hisseorani")
        hissepayadeti = request.POST.get("hissepayadeti")
        hissetutari = request.POST.get("hissetutari")
        ticaretsicilno = request.POST.get("ticaretsicilno")
        ticaretsicilgazetetarihi = request.POST.get("ticaretsicilgazetetarihi") 
        ticaretsicilgazeteno = request.POST.get("ticaretsicilgazeteno")
        ticaretsicilgazetesayfano = request.POST.get("ticaretsicilgazetesayfano")
        vergidairesi = request.POST.get("vergidairesi")
        vergino = request.POST.get("vergino")
        vergidairesikodu = request.POST.get("vergidairesikodu")
        vergidairesiili = request.POST.get("vergidairesiili")
        vergidairesiilcesi = request.POST.get("vergidairesiilcesi")
        bagkurno = request.POST.get("bagkurno")
        bagkurbasamagi = request.POST.get("bagkurbasamagi")
        sskno = request.POST.get("sskno")
        emeklisandigino = request.POST.get("emeklisandigino")
        digerisyerisicilno = request.POST.get("digerisyerisicilno")
        vakifno = request.POST.get("vakifno")
        dernekno = request.POST.get("dernekno")
        tututugudefterturu = request.POST.get("tututugudefterturu")
        meslegi = request.POST.get("meslegi")
        faliyetkodu = request.POST.get("faliyetkodu")
        faliyetsuresi = request.POST.get("faliyetsuresi")
        meslegitesekkuladi = request.POST.get("meslegitesekkuladi")
        meslegitesekkulno = request.POST.get("meslegitesekkulno")
        aracplakano1 = request.POST.get("aracplakano1")
        aracplakano2 = request.POST.get("aracplakano2")
        aracplakano3 = request.POST.get("aracplakano3")
        ikamatgahadresi = request.POST.get("ikamatgahadresi")
        ikamatgahbulvar = request.POST.get("ikamatgahbulvar")
        ikamatgahcadde = request.POST.get("ikamatgahcadde")
        ikamatgahsokak = request.POST.get("ikamatgahsokak")
        ickapi = request.POST.get("ickapi")
        diskapi = request.POST.get("diskapi")
        ikamatgahmahkoy = request.POST.get("ikamatgahmahkoy")
        il = request.POST.get("il")
        ilcesi = request.POST.get("ilcesi")
        postakodu = request.POST.get("postakodu")
        isyeriadresi = request.POST.get("isyeriadresi")
        isyeriil = request.POST.get("isyeriil")
        isyeriilcesi = request.POST.get("isyeriilcesi")
    return render (request,"firma_durumlari/ortak_durumu.html",content)