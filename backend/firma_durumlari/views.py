...
def firmaya_sube_ekleme(request,slug):
    ...
    if request.method == "POST":
        ...
        yeni_sube = sube.objects.create(
            bagli_oldugu_firma = get_object_or_404(firma,firma_ozel_anahtar = slug ),
            sube_adi = sube_adi,sube_unvani = sube_soyadi,adres_bilgisi = get_object_or_404(adresler,id = yeni_adres.id),
            vergi_dairesi_adi =get_object_or_404(vergi_dairesi,vergi_dairesi_adi = firmavergidairesi) ,vergi_dairesi_kodu = firmavergidairesikodu,
            vergi_numarasi = firmaverginumarasi,sahis_ise_tc = sahisisetc,
            email_adresi = firmaeposta,web_adresi = firmawebadresi,
...
def firma_ortak_eklme_silme(request,slug):
    ...
    content["sgkbm"] = sgk_bolgecalisma_mudurlukleri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    content["iskur"] = iskurbilgileri.objects.filter(sube_bilgisi__id = sube_bilgisi.id).first()
    content["ortaklar"] = ortak_bilgileri.objects.filter(firma_bilgisi = get_object_or_404( firma,silinme_bilgisi = False,firma_ozel_anahtar = slug))
    sgkpersonelkanun = sgk_kanunlari.objects.all()
    content["sgkpersonelkanun"]=sgkpersonelkanun
    if request.POST:
